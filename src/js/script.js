window.addEventListener("load", () => {
    setTimeout(() => {
      ...
      document.getElementById("middle-video").classList.add("active");
    }, 500);
  });
  ...
  function applyMagnet(x, y) {
    ...
    const offsetX = x - (boundingRect.left + boundingRect.width / 2);
    const offsetY = y - (boundingRect.top + boundingRect.height / 2);
  ...
  tv.addEventListener('mouseenter', () => {
    boundingRect = tv.getBoundingClientRect();
  });
  ...
  tv.addEventListener('mouseleave', resetMagnet);

  // Mobile
  tv.addEventListener('touchstart', (e) => {
    boundingRect = tv.getBoundingClientRect();
    const touch = e.touches[0];
    applyMagnet(touch.clientX, touch.clientY);
  });
  ...
  tv.addEventListener('touchend', resetMagnet);
  tv.addEventListener('touchcancel', resetMagnet);
