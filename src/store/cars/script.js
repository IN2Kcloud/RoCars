...
// Find car data
const car = carDetails.find(item => item.slug === slug);

// DOM elements
const modelNameEls = document.querySelectorAll(".model-name");
...
const carHighlight = document.querySelector(".car-highlight");
const imgPreview = document.querySelector(".img-preview img");
const minimapItems = document.querySelector(".minimap .items");

// Render car info if available
if (car) {
  modelNameEls.forEach(el => el.textContent = car.name);
  carYear.textContent = `Year: ${car.year}`;
  carPrice.textContent = `Price: ${car.price}`;
  carHighlight.textContent = car.highlight;
  ...
  if (car.images.length > 0) {
    imgPreview.src = car.images[0];
    imgPreview.alt = `${car.name} preview`;
  }
  ...
  car.images.forEach((src, index) => {
    ...
    thumb.src = src;
    thumb.alt = `${car.name} thumbnail ${index + 1}`;
    thumb.classList.add("minimap-thumb");
    ...
    minimapItems.appendChild(thumb);
  });
} else {
  document.querySelector(".car-details").innerHTML = "<p>Car not found.</p>";
}
*/